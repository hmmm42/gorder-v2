...
func PublishEvent(ctx context.Context, p PublishEventReq) (err error) {
	...
	switch p.Routing {
	default:
		logging.Panicf(ctx, nil, "unsupported routing type: %s", string(p.Routing))
	case FanOut:
		return fanOut(ctx, p)
	case Direct:
...
func doPublish(ctx context.Context, ch *amqp.Channel, exchange, key string, mandatory, immediate bool, msg amqp.Publishing) error {
	if err := ch.PublishWithContext(ctx, exchange, key, mandatory, immediate, msg); err != nil {
		logging.Warnf(ctx, nil, "_publish_event_failed||exchange=%s||key=%s||msg=%s", exchange, key, msg)

...
function gen_for_modules {
  ...
  if [ -d "$go_out" ]; then
    log_warning "found existing $go_out, removing"
    run rm -rf "$go_out"
  fi
  ...
  for dir in $(dirs); do
    ...
    if [ -d "$go_out/$dir" ]; then
      log_warning "found existing $go_out/$dir, removing"
  ...
  log_success "generated code for all services"
}

echo "directories containing protos to be built: $(dirs)"
echo "found proto files: $(pb_files)"
gen_for_modules